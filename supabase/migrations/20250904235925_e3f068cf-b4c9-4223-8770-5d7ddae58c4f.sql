-- Extend the app_role enum to include the new roles
ALTER TYPE public.app_role ADD VALUE 'pm' AFTER 'admin';
ALTER TYPE public.app_role ADD VALUE 'ar1_planning' AFTER 'pm';
ALTER TYPE public.app_role ADD VALUE 'ar2_field' AFTER 'ar1_planning';
...
USING (
  ...
  (has_role(auth.uid(), 'ar1_planning') AND project_milestones.ar_assigned = (
...
USING (
  ...
  (has_role(auth.uid(), 'ar1_planning') AND ar_assigned = (
    SELECT full_name FROM public.profiles WHERE user_id = auth.uid()
  ))
);
