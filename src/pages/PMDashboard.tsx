...
const PMDashboard = () => {
  ...
  useEffect(() => {
    if (!loading && !isPM && !isAdmin) {
      navigate('/');
    }
  }, [isPM, isAdmin, loading, navigate]);

  // Fetch current user's name for PM matching
  useEffect(() => {
  ...
  const fetchPMProjects = async () => {
    try {
      setLoading(true);
      console.log("Fetching PM projects...");

      // Fetch projects where PM is owner OR assigned as PM
      ...
      // Apply PM-specific filtering (not admin sees all)
      ...
      console.log("PM Projects fetched:", projectsData?.length || 0);
